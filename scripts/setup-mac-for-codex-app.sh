#!/usr/bin/env bash
# Make Something — macOS / Linux / WSL Setup (for Codex App users)
# Run with: bash <(curl -fsSL https://raw.githubusercontent.com/filip-pilar/makesomething/main/scripts/setup-mac-for-codex-app.sh)
#
# Idempotent — safe to run multiple times.
# This version is for users who have the Codex desktop app (no CLI installation).
...
echo "  ✓ Setup complete!"
echo ""
echo "  Your app is now running in the browser at http://localhost:3000"
echo ""
echo "  Next steps:"
echo "    1. Open the Codex app"
echo "    2. Navigate to: $INSTALL_DIR"
echo "    3. Start building!"
echo ""
echo "  Press Ctrl+C to stop the dev server when you're done."
echo ""

# Keep the script running so the dev server stays alive
wait $DEV_PID
